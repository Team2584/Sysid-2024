...
void SysIdRoutineBot::ConfigureBindings() {
  ...
  (m_driverController.A() && m_driverController.LeftTrigger())
      .WhileTrue(m_intake.SysIdQuasistatic(frc2::sysid::Direction::kForward));
  (m_driverController.B() && m_driverController.LeftTrigger())
      .WhileTrue(m_intake.SysIdQuasistatic(frc2::sysid::Direction::kReverse));
  (m_driverController.X() && m_driverController.LeftTrigger())
      .WhileTrue(m_intake.SysIdDynamic(frc2::sysid::Direction::kForward));
  (m_driverController.Y() && m_driverController.LeftTrigger())
